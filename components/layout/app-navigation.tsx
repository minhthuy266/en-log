...
const items = [
  ...
  { label: "Mock Tests", href: "/mock-tests", short: "Tests" },
...
export function AppNavigation({ email }: { email: string }) {
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 hidden w-64 border-r bg-card md:flex md:flex-col">
      <div className="border-b p-5"><p className="font-semibold text-primary">TOEIC 900+ Log</p><p className="mt-1 truncate text-xs text-muted-foreground">{email}</p></div>
    ...
    <header className="sticky top-0 z-20 border-b bg-card/95 px-4 py-3 backdrop-blur md:hidden"><p className="font-semibold text-primary">TOEIC 900+ Log</p></header>
