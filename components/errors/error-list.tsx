...
import { captureReasonLabels, errorTypeLabels, errorTypes, partLabels, toeicParts } from "@/lib/constants";
import { useErrors } from "@/lib/queries/use-errors";

export function ErrorList() {
  const query = useErrors(); const [search, setSearch] = useState(""); const [part, setPart] = useState(""); const [type, setType] = useState(""); const [reason, setReason] = useState("");
  const errors = useMemo(() => (query.data ?? []).filter((item) => (!part || item.toeic_part === part) && (!type || item.error_types.includes(type as typeof item.error_types[number])) && (!reason || item.capture_reason === reason) && [item.question_text, item.explanation ?? "", item.source_name ?? "", ...item.rules.map((rule) => rule.title)].join(" ").toLowerCase().includes(search.toLowerCase())), [query.data, part, type, reason, search]);
