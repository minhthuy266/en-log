...
function RuleDetail({ rule, editing, onEditing, onDelete }: { rule: RuleWithStats; editing: boolean; onEditing: (value: boolean) => void; onDelete: () => void }) {
  return <div className="grid gap-5 rounded-xl border bg-card p-4 md:p-5">
    ...
    <div><h3 className="text-sm font-semibold">Linked errors ({rule.questions.length})</h3><div className="mt-3 grid gap-2">{rule.questions.slice(0, 8).map((q) => <div className="rounded-lg border p-3 text-sm" key={q.id}><p className="line-clamp-2">{q.question_text}</p><p className="mt-1 text-xs capitalize text-muted-foreground">{q.section.replace("_", " ")} · {q.occurred_on}</p></div>)}{!rule.questions.length && <p className="text-sm text-muted-foreground">No linked errors.</p>}</div></div>
  </div>;
}
