...
export function AnalyticsView() {
  ...
  const partData = [...data.byPart].sort((a, b) => a.key.localeCompare(b.key));
  return <div className="grid gap-6">
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric label="Logged errors" value={data.totalErrors} /><Metric label="Due rules" value={data.dueRules} /><Metric label="Repeated rules" value={data.repeatedRules} /><Metric label={data.gapTo900 === 0 ? "Target" : "Gap to 900"} value={data.latestScore === null ? "—" : data.gapTo900 === 0 ? "900+" : (data.gapTo900 ?? "—")} /></div>
    ...
    <div className="grid gap-6 lg:grid-cols-2"><ChartCard title="Logged friction by part" note="Counts are not accuracy; only selected errors are logged."><ResponsiveContainer height="100%" width="100%"><BarChart data={partData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" fontSize={12} /><YAxis allowDecimals={false} fontSize={12} /><Tooltip /><Bar dataKey="count" fill="hsl(var(--primary))" radius={[5,5,0,0]} /></BarChart></ResponsiveContainer></ChartCard><ChartCard title="Top error types" note="One question may count in multiple types."><ResponsiveContainer height="100%" width="100%"><BarChart data={data.byErrorType.slice(0, 6)} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis allowDecimals={false} type="number" /><YAxis dataKey="label" fontSize={11} type="category" width={110} /><Tooltip /><Bar dataKey="count" fill="hsl(var(--primary))" radius={[0,5,5,0]} /></BarChart></ResponsiveContainer></ChartCard></div>
    {data.mockTests.length > 0 && <ChartCard title="Mock score trend" note="Judge the rolling trend; one test form can fluctuate."><ResponsiveContainer height="100%" width="100%"><LineChart data={data.mockTests}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="taken_on" fontSize={11} /><YAxis domain={[0, 990]} /><Tooltip /><Legend /><Line dataKey="listening_score" name="Listening" stroke="#2563eb" strokeWidth={2} /><Line dataKey="reading_score" name="Reading" stroke="#7c3aed" strokeWidth={2} /><Line dataKey="total_score" name="Total" stroke="#059669" strokeWidth={3} /></LineChart></ResponsiveContainer></ChartCard>}
